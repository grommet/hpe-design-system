/**
 * Do not edit directly, this file was auto-generated.
 */

export default {
  "hpe": {
    "color": {
      "background": {
        "default": "#ffffff",
        "back": "#f7f7f7",
        "front": "#ffffff",
        "floating": "#ffffff",
        "contrast": "rgba(0, 0, 0, 0.04)",
        "hover": "rgba(0, 0, 0, 0.04)",
        "active": "rgba(0, 0, 0, 0.04)",
        "disabled": "rgba(0, 0, 0, 0.04)",
        "screenOverlay": "rgba(0, 0, 0, 0.12)",
        "ok": "rgba(23, 235, 160, 0.24)",
        "critical": "rgba(252, 97, 97, 0.24)",
        "unknown": "rgba(0, 0, 0, 0.04)",
        "info": "rgba(0, 200, 255, 0.24)",
        "warning": "rgba(255, 188, 68, 0.24)",
        "primary-default": "#01a982",
        "primary-hover": "#01a982",
        "selected-strong-enabled": "#01a982",
        "selected-strong-hover": "#01a982",
        "selected-weak-enabled": "#cbfaeb",
        "selected-weak-hover": "#aef6df",
        "neutral-xstrong": "#2e2e2e"
      },
      "border": {
        "strong": "rgba(0, 0, 0, 0.72)",
        "default": "rgba(0, 0, 0, 0.36)",
        "weak": "rgba(0, 0, 0, 0.12)",
        "disabled": "rgba(0, 0, 0, 0.12)",
        "selected": "#01a982",
        "critical": "rgba(0, 0, 0, 0.36)",
        "info": "rgba(0, 0, 0, 0.36)",
        "ok": "rgba(0, 0, 0, 0.36)",
        "warning": "rgba(0, 0, 0, 0.36)",
        "unknown": "rgba(0, 0, 0, 0.36)"
      },
      "text": {
        "default": "#555555",
        "strong": "#2e2e2e",
        "weak": "#757575",
        "placeholder": "#757575",
        "disabled": "rgba(0, 0, 0, 0.24)",
        "onPrimary": "#ffffff",
        "primary": "#006750",
        "critical": "#555555",
        "info": "#555555",
        "ok": "#555555",
        "warning": "#555555",
        "unknown": "#555555",
        "heading-default": "#2e2e2e",
        "onSelectedStrong": "#ffffff",
        "onSelected": "#2e2e2e",
        "onStrong-default": "#ffffff",
        "onCritical-strong": "#2e2e2e",
        "onCritical-default": "#555555",
        "onInfo-strong": "#2e2e2e",
        "onInfo-default": "#555555",
        "onOk-strong": "#2e2e2e",
        "onOk-default": "#555555",
        "onUnknown-strong": "#2e2e2e",
        "onUnknown-default": "#555555",
        "onWarning-strong": "#2e2e2e",
        "onWarning-default": "#555555"
      },
      "icon": {
        "default": "#555555",
        "strong": "#2e2e2e",
        "weak": "#757575",
        "disabled": "rgba(0, 0, 0, 0.24)",
        "critical": "#ec3331",
        "info": "#00c8ff",
        "ok": "#009a71",
        "warning": "#d36d00",
        "unknown": "#8c8c8c",
        "primary": "#01a982",
        "onPrimary": "#ffffff",
        "onSelectedStrong": "#ffffff",
        "onSelected": "#2e2e2e"
      },
      "decorative": {
        "brand": "#01a982",
        "green": "#17eba0",
        "purple": "#f740ff",
        "teal": "#82fff2",
        "blue": "#00c8ff",
        "red": "#fc6161",
        "orange": "#ffbc44",
        "yellow": "#ffeb59",
        "purple!": "#7630ea",
        "green!": "#01a982",
        "teal!": "#00e8cf",
        "blue!": "#00739d",
        "red!": "#c54e4b",
        "orange!": "#ff8300",
        "yellow!": "#fec901"
      },
      "dataVis": {
        "categorical-10": "#01a982",
        "categorical-20": "#00384d",
        "categorical-30": "#bd8d09",
        "categorical-40": "#7022ec",
        "categorical-50": "#3e92cc",
        "categorical-60": "#a70179",
        "categorical-70": "#01609f",
        "categorical-80": "#7800a7"
      },
      "transparent": "rgba(0, 0, 0, 0)",
      "focus": "#00e8cf",
      "foreground": {
        "primary": "#01a982",
        "critical": "#ec3331",
        "warning": "#d36d00",
        "unknown": "#8c8c8c"
      }
    },
    "shadow": {
      "small": "0 4px 4px rgba(0, 0, 0, 0.12) ",
      "medium": "0px 6px 12px 0px rgba(0, 0, 0, 0.12) ",
      "large": "0px 12px 24px 0px rgba(0, 0, 0, 0.24) "
    }
  }
}
