...
let contributorCommitMessages = function(topic, currentFileName){
    ...
    contrMsgs.forEach(element => {
        ...
        let oneYearAgo = new Date(new Date().setFullYear(new Date().getFullYear() - 1)).getTime();
        let commitDate = new Date(element.commit.author.date).getTime()
        if(commitDate > oneYearAgo){
            let temp = {date: new Date(element.commit.author.date).toDateString(), string: element.commit.message};
