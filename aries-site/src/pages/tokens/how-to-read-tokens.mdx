...
While our documentation uses `.` dot as a delimiter, token parts should be separated by the character is appropriate for a given framework. For example, use the `-` dash character for CSS variables.
