...
More detailed information can be found on the [Status Indicator Design System Page](/templates/status-indicator).

A State is the current stage of a process. A Status is the condition of the State. Status can also be thought of as a pointer to a State’s “health”.
...
<ExampleImagePreview
  label="DataTable with separate columns for Status and State"
  href="#datatable-with-separate-columns-for-status-and-state"
...
<ExampleImagePreview
  label="DataTable with combined column for Status and State"
  href="#datatable-with-combined-column-for-status-and-state"
...
<ExampleImagePreview
  label="DataTable with only a Status column"
  href="#datatable-with-only-a-status-column"
  src="/static/images/datatable-only-status-column.svg"
  alt="Example showing a status column in the data table and an arrow pointing to the state within the expanded details"
/>
