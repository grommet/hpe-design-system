...
The `badge` prop adds a small indicator to a Button to show extra information, such as a notification or message count. It can be used in different ways depending on what needs to be communicated:
...
- For large numbers, an object with value and max can be used. If the value exceeds the max, the badge will display a "+" next to the max (e.g., 9+), keeping the visual presentation clean and readable. By default the max is 9, but this can be adjusted based on what max value best suits the use case.
