...
| **New** | "New" opens a template, or blank canvas, of a given object type to allow for user input or content. The object will not be added as a record in a database until a "Create" action has been completed. | When starting the configuration process for a new device, use "New device" to initiate the process. |
| **Create** | "Create" is typically used for form submission, to submit user input and create an instance of an object. | After completing inputs related to configuring new device, use "Create device" to submit the data and create the object instance. |
