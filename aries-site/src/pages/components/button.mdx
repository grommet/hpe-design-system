...
Toolbar buttons are used when buttons are placed in a row with other toolbar controls. The most common use case is in a filtering context when there is a search input, a filter button, and an action menu. The intent is to give each control equal prominence. To implement, apply `kind="toolbar"` to Button or Button-based components like Menu.
