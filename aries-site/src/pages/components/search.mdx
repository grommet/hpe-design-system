...
Place the icon in the position that most helps people to scan the page and identify the search field. Working as a visual aid, the search icon is best seen before the placeholder text. When deciding where to place the icon, consider where the search bar is located and how the user might be scanning the page (for example, in an F pattern from left to right).
