...
| ----- | ---------------- | ----------------- | :-----------------------------------------: | ----------------- |
| **1** | **Close button** | Closes the layer. | <Checkmark a11yTitle="true" size="small" /> | Use "Close" icon. |
| **2** | **Apply button**| Apply filtered items. | <Checkmark a11yTitle="true" size="small" /> | The footer action should be left aligned with primary action on the left. Having the `onClickOutside` provides the user a way to exit if they do not wish to have filters applied. |
