...
This diagram highlights details specific to side drawer layers. See [general layer anatomy](/components/layer#anatomy) for specs that apply to all layers.
...
1.  **Footer actions**: The actions should be left-aligned with primary action on the left. One action should be a "Cancel" button that closes the layer. See [layer closing behavior](/components/layer#actionable) for details.
