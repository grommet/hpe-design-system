...
See [general layer anatomy](/components/layer#anatomy) for specs that apply to all layers.

1.  **Close button** (only on layers with no footer actions): A button to close the layer.
1.  **Footer actions**: Because the layer is anchored to the right edge of the screen, the actions should be left-aligned with primary action on the left. One action should be a "Cancel" button that closes the layer. See [layer closing behavior](/components/layer#layer-closing-behavior) for details.
