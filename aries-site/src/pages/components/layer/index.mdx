...
Depending on if a layer is informational or actionable, the elements and interactions that will close a layer differ.

### Informational

Informational layers do not require any actions from the user, and therefore do not contain any footer actions.

The following should cause an informational layer to close:

1. Clicking on the close button in the layer's header.
...
Actionable layers require the user to either accept/apply the prompt in the layer or cancel out.

The following should cause an actionable layer to close:

1. Clicking the close button in the layer's header.
   - The exception to this rule is that actionable center layers should not include a close button in the layer's header. Because center layers generally present confirmations, the user should have to intentionally choose between accepting or cancelling the layer's prompt.
1. Clicking the cancel button in the layer's footer.
1. Pressing the Escape key, `onEsc`.

Clicking outside of an actionable layer should not close the layer. The user should be intentionally choosing between accepting or cancelling the layer's prompt.
...
When data has been edited or changed and the user tries to close the layer, a double confirmation should be shown.
