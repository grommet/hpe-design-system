...
- For situations where the contents exceed the height of the layer, the header should either always stick atop the Layer while the content scrolls beneath, or to maximize screen real-estate, the header may scroll away as the user scrolls down the content, but re-appear as the user scrolls up, signaling their intent to return to the top of the content.
...
The Layer header and action buttons can be sticky while the rest of the Layer content is scrollable.

<Example
  docs="https://v2.grommet.io/layer?theme=hpe#props"
  code="https://raw.githubusercontent.com/grommet/hpe-design-system/master/aries-site/src/examples/components/layer/LayerFormExample.js"
  height={{ min: 'small' }}
>
  <LayerFormExample />
</Example>
...
In order maximize screen real-estate, the header may scroll away as the user scrolls down the content, but re-appear as the user scrolls up.
