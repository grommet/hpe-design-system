...
Treat a Layer as if it were a stand alone page. Therefore, the first heading within a Layer should be a Heading with `level={1}`, and all subsequent headings should follow typical [semantic heading structure](/foundation/typography#semantic-usage-of-heading-levels).
...
```
<Button 
  a11yTitle={`You are in a layer with a form to add new hosts. 
