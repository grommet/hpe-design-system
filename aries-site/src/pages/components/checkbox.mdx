...
In product design there is usage for a single checkbox that may be needed to turn a feature on or off.
At times a single Checkbox may control progressiive disclosure which works well if the checkbox is not wrapped in a FormField.
