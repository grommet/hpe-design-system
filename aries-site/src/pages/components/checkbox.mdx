...
The CheckBox has a built in `pad` to allow space when wrapped in a FormField. In order to have a stand alone CheckBox align with other FormFields in a Form users can explicatly 
set the `pad: {{ vertical: 'xsmall' }}` in order to remove the horizontal pad. 
