...
By default, CheckBox has built-in `pad` to create space between the CheckBox and FormField border therefore when using checkbox alone `pad="none"` will need to be applied.
When using CheckBox outside of FormField, it does not have a border or label. The hover indicator is applied to the checkbox itself.
