...
1. When there is progressive disclosure: i.e. clicking on the checkbox will reveal related options that wouldn't have been displayed unless the user checked the box.
2. To turn a feature on or off also consider using a [toggle button](https://design-system.hpe.design/components/button#toggle-buttons)
3 .There is no additional information or help text necessary for the user to understand what is being asked of them.
4. Allowing a user to opt-in or acknowledge a disclaimer, such as a privacy policy or terms and conditions.

By default, CheckBox has built-in `pad` to create space between the CheckBox and FormField border. When using CheckBox outside of a FormField, apply `pad="none"`.
