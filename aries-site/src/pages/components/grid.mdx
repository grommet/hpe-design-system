...
- [Fixed](#fixed-grid) column widths and row heights, when use cases desire precise dimensions for grid cells.
- [Flexible](#flexible-grids) column widths and row heights, allowing grid cell contents to gracefully respond 
...
With flexible columns, columns are defined as a min-max range. Between those bounds, the column is 
...
be applying filters to a catalog of services, presenting a dashboard where the user may customize the 
included widgets, or image results returned from a search query.
