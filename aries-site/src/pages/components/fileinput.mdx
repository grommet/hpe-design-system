...
<Example code="https://raw.githubusercontent.com/grommet/hpe-design-system/master/aries-site/src/examples/components/fileinput/FileInputValidationExample.js">
  <FileInputValidationExample />
</Example>
...
<Example code="https://raw.githubusercontent.com/grommet/hpe-design-system/master/aries-site/src/examples/components/fileinput/FileInputConfirmRemoveExample.js">
  <FileInputConfirmRemoveExample />
</Example>
...
At the moment, we are recommending that implementers use the [renderFile prop](https://v2.grommet.io/fileinput#renderFile) to display the full file name in cases where truncation occurs and the full name needs to be visible. The default tooltip uses the native HTML title attribute, which is not accessible to keyboard users. Since this is a browser-level limitation and not an issue with Grommet itself, we're unable to address it directly.

```
<FileInput
  renderFile={(file) => <Text>{file.name}</Text>}
/>
```
