...
- Use the right set of columns, prioritizing frequently used ones. Don't add more columns just because the data is there, as this will clutter the table.
- To support readability, make tables as wide as they need to be and no bigger. If the content does not need a large width, don't let the table stretch.
...
- When the table has the potential to be wider than the screen:
  ...
  - Use horizontal scrolling within the table, not at the level of the entire page
- When the table has the potential to be narrower than its layout context:
  - Align it horizontally with the rest of its layout context, typically aligned to the start of the content as opposed to stretched
  - Align any header above the table, including heading, search, filter, actions, in the same way. This prevents an orphaned actions menu/button out to the right away from the table. But, don't force the header above the table to align its width to the table, as that might compress the contents of the header too much.
