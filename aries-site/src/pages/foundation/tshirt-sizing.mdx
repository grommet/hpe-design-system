...
- **Has parity with existing scale** which means that all designs may be recreated with the new scale without any layout shifts except for example container `xlarge` or 1152px doesn't have an equivalent size on the new scale. We recommend using `xxlarge` or 1024px instead on the new scale.
...
Given the [Container sizes comparison](#container-sizes-comparison-width-height-size-columns-rows) table, if you were to implement the following code, you would have a first Box with width of "384px" and height of "192px", second Box with width of "384px" and height of "384px" and a third Box with width of "384px" and height of "768px"
