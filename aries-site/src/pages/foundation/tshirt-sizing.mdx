...
- In the new scale, `medium` matches the v6 `medium`. The remaining t-shirt sizes require mapping to achieve design parity. Although this is not the case for `round` as shown on [Radius sizes comparison](#radius-sizes-comparison-round) table.
