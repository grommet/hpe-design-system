...
### Original scale (v6 and below)

Below is the original sizing scale from Grommet's [`base.js`](https://github.com/grommet/grommet/blob/master/src/js/themes/base.js) theme file that was used in HPE theme v6 and earlier versions to drive the height/width for components such as [Box](https://design-system.hpe.design/components/box). The `base.js` theme establishes many of the defaults leveraged across Grommet applications.
...
### Evolution to v7: The "Dual Doubling" Scale
...
- **Scale consists of a primary scale** based on the doubling of a 24px base unit (same as v6) + a supplementary scale doubling a 16px base unit

**Results in the following pixel progression:**
1, 2, 3, 4, 6, 8, 12, 16, 24, 32, 48, 64, 96, 128, 192, 256, 384, 512, 768, 1024, 1536, 2048

Below are the sizing values from the [HPE theme v7](https://github.com/grommet/grommet-theme-hpe/blob/master/src/js/themes/hpe.js) and above which implements this expanded scale:
...
Given the code snippet from the previous section and the explanation on HPE theme v7 t-shirt sizing for dimensions, if you were to implement the following code, you would have a Box (first) with width of "384px" and height of "192px".
...
## Migrating to HPE Theme v7

The transition to v7 brings expanded sizing options but requires mapping existing sizes to new equivalents. The key principle is that `medium` remains unchanged, but other sizes need to be remapped:

### Size mapping reference

**For spacing properties** (pad, margin, gap, thickness):
...
**For container properties** (width, height, size, columns, rows):
...
For automated migration assistance, use the [HPE Design System codemod](https://github.com/grommet/hpe-design-system/tree/master/packages/codemods) which handles the majority of migration scenarios.
