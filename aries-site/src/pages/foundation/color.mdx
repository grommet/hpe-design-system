...
when using `text-strong` <ColorSwatch background={{color: "text-strong", dark: false}} /> color.
This is ensured for text of size
18px and above.

If a font size is below 18px, AAA compliance will fail with
default font weight on an HPE `green` <ColorSwatch background={{color: "green", dark: false}} />
...
when using `text-strong` <ColorSwatch background={{color: "text-strong", dark: true}} /> color.
To meet AAA compliance, use text
