...
Typography refers to the creation and styling of letters into fonts. The HPE brand font is MetricHPE. Our typography is about more than words. It’s an integral part of our personality and design. When we’re making a statement, we want our visual language to be clear, recognized and understood.
...
The weights and styles shown are part of the HPE Design System theme. MetricHPE comes in a varity of weights. For digital experiences we use Light, Regular, Medium, and Bold weights at a variety of scales to complement the content in a given design.
...
For accessibility reasons, always ensure to apply [semantically correct heading levels](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/Heading_Elements#Usage_notes).
...
- Apply Heading with `level={2}`to Layers.
...
If Heading `size` is adjusted, it should be adjusted for all levels. For example, if `size="small"` is desired for a Heading level 1, use "small" for the size of all levels.
