...
export const card = [getPageDetails('WCAG Checklist')];
...
The following checklist is a resource for researchers, designers and developers to use to check their product's accessibility. It uses the Web Content Accessibility Guidelines (WCAG) as a reference point, as the WCAG is a shared accessibility standard for individuals, organizations and governments worldwide.
