...
import { CollapsedStateAnatomy, CreateCluster, ExpandedStateAnatomy } from '../../../examples/templates/forms';
...
| **4** | **Values summary** | A preview summarizing the object's detail.                 | Optional |       |
| <Annotation target="5" kind="style" /> | **Border** |                                   | <Checkmark a11yTitle="true" size="small" /> | color="border-weak"      |      
...
| **5** | **Remove button** | Removes/deletes the child from its parent.                  | Case depending | <span>Always provide unless at least one child object is required by the parent. See [Required children](#required-children) for detail.</span> Use the verb "remove" when removing the association between parent and child. Reserve the verb "delete" for actions where the child will no longer exist. | 
