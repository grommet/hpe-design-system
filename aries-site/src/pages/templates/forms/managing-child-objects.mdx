...
For use cases where the user needs the ability to remove the parent object entirely, there are two options available; the nested placement and top-level placement.

#### Nested placement

The nested placement is the standard option for removing a parent object. It avoids competing with the visual anchor of the object header and prevents users from accidentally removing the object.
