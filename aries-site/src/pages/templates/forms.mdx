...
- Write helpful placeholder text inside the fields if there is a high chance the user would need clarity on the format (for example, MM/DD/YYYY) or deciding what to write.
...
Good forms make it intuitive for the user to understand what is required vs what is optional. Adding too much on the form - even if the intention is to be helpful - can create clutter and confusion.  
...
Some forms, such as signup and signin forms need no explanations. And while we aim for specific field labels, you can use a placeholder text inside fields where the user might need some extra help in deciding how to format their answer.  
