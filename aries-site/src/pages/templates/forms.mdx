...
- Use clear, concise title labels for each field. 
- Write helpful placeholder text inside the fields. 
- If the user would likely have doubts, include helpful text below the field, such as [password requirements](#signup) or [additional resources](#customize) on a field's context.
...
- Forms with minimal fields are optimal but occasionally, capturing extra information would be highly beneficial for the user. The following are some options for addressing these cases: 
   ...
   - Create the form the optimal way; with only the minimum required fields so that the user can focus on completing their task. Separately, in a preferences or settings dashboard, for example, create those fields where the user can proactively add this helpful information at another time. For example, asking for someone's official title or role might be highly useful on a sign-in form. However, this might also be a charged question with many implications for the admin to answer easily. Instead, offer this "role" or "title" field in profile settings.
