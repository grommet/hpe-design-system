...
Users can search for information in which will allow the user to quickly naviagte to the desired screen. A user can also 
view information within a set of cards to allow them to click on the card to view the information as a whole.

<Example 
  caption="Illustration showing how a user has flexabilty with navigating through an application by navigating to certain applications and then being able to view their profile and 
    which can be done in no particular order." 
  plain 
  pad="small" 
  previewWidth="large" 
  height={{ min: 'small' }}
