...
Validation should occur within each step of the wizard. This can happen `onBlur` of each input or when the user clicks the button that would advance them to the next step. If there are any errors with the inputs in the current step, the user should not be able to advance.
