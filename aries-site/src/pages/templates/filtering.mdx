...
- In most cases when there are many available filters, they should be [presented in a layer](#filters-in-a-layer).
- At times, it may be helpful to expose a [frequently used filter in the toolbar](#exposed-filter-in-toolbar) directly.

Different types of inputs may be better suited for filtering different types of data.
...
If it's expected that a specific filter will be most frequently used, it can be useful to expose it in the toolbar.
...
The summary displayed beneath the toolbar should always reflect the user's actions. When filtering is combined with selection, the summary text should adjust for the following scenarios.
