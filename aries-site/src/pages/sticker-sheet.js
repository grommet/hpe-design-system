...
import { hpe as hpeCurrent } from 'grommet-theme-hpe';
...
export const current = deepMerge(hpeCurrent, {
  global: {
    font: {
      family: 'Metric',
    },
  ...
  button: {
    extend: props =>
      `${hpeCurrent.button.extend(props)} font-family: 'Metric';`,
  },
  paragraph: {
    extend: props =>
      `${hpeCurrent.paragraph.extend(props)} font-family: 'Metric';`,
  },
  text: {
    extend: props => `${hpeCurrent.text.extend(props)} font-family: 'Metric';`,
  },
});
...
const Compare = ({ children, ...rest }) => {
  ...
  return (
    <Stack {...rest}>
      ...
      <Box
        align="start"
        style={mode === 'v5.1.1' ? { visibility: 'hidden' } : {}}
      >
        {children}
      </Box>
    </Stack>
            ...
            current theme to the next.`}
              actions={
                ...
                <FormField label="View mode">
                  <Select
                    options={['v5.1.1', 'Metric 2', 'Compare diffs']}
                    value={mode}
