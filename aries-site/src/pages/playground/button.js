...
const generateCode = ({
  ...
  if (iconName) {
    imports.unshift(`import { ${iconName} } from '@hpe-design/icons-grommet';`);
  }
...
export default function ButtonPlayground() {
  ...
  const [badgeMode, setBadgeMode] = useState('dot'); // 'dot' | 'value'
  ...
  // active   – visual "selected" indicator. Turning it ON clears disabled
  //            (an active button is by definition interactive). Can coexist
  //            with busy and success.
  //
  // disabled – removes interactivity entirely. Turning it ON clears active,
  //            busy, and success because those states only make sense on a
  //            button the user can interact with or has just interacted with.
  //
  // busy     – feedback that an async action is in progress. Mutually exclusive
  //            with success (can't be both at once) and with disabled (an action
  //            can't be in-progress on a non-interactive button). Turning it ON
  //            clears success and disabled.
  //
  // success  – feedback that an action completed. Mutually exclusive with busy
  //            and disabled for the same reasons above. Turning it ON clears
  //            busy and disabled.
  //
  // Note: busy and success are suppressed entirely when href is set, because
  // anchor buttons navigate rather than trigger async actions.
  // ---------------------------------------------------------------------------
  const toggleBusy = (checked) => {
    setBusy(checked);
    if (checked) {
      setSuccess(false);
      setDisabled(false);
  ...
  const tipDropProps =
    tipEnabled ? TIP_POSITION_OPTIONS[tipPositionIndex].value : undefined;
  ...
  const previewProps = useMemo(() => {
    ...
    if (tipEnabled) {
      p.tip = tipContent
        ? { content: tipContent, dropProps: tipDropProps }
        : 'Tooltip';
    }

    if (badgeEnabled) {
      p.badge = badgeMode === 'dot' ? true : { value: badgeValue, max: badgeMax };
    }
    ...
    <Grommet theme={hpe} full>
      <Layer full modal={false} responsive={false}>
        ...
        >
          ...
          {/* RIGHT PANEL — full height, scrollable controls */}
          <Box
            gridArea="side"
            ...
            style={{ minHeight: 0, minWidth: 0 }}
          >
            {/* ✅ Form provides correct inter-field spacing in HPE theme */}
            <Form gap="medium" onSubmit={(e) => e.preventDefault()}>
              ...
              <NestedBox>
                <CheckBox
                  ...
                  onChange={(e) => {
                    ...
                    setIconOnly(checked);
                    if (checked && !iconName) setIconName('Add');
                  }}
                />
              </NestedBox>
              ...
              <Box gap="3xsmall">
                ...
                {!href && (
                  <CheckBox
                    label="Success"
                    checked={success}
                    onChange={(e) => toggleSuccess(e.target.checked)}
                  />
                )}
              </Box>
              ...
              {badgeEnabled && (
                <NestedBox>
                  <Box direction="row" gap="small">
                    <CheckBox
                      label="Dot only"
                      checked={badgeMode === 'dot'}
                      onChange={() => setBadgeMode('dot')}
                    />
                    <CheckBox
                      label="Show value"
                      checked={badgeMode === 'value'}
                      onChange={() => setBadgeMode('value')}
                    />
                  </Box>
                  {badgeMode === 'value' && (
                    <>
                      <FormField label="Value">
                        <TextInput
                          ...
                          value={badgeValue}
                          onChange={(e) =>
                            setBadgeValue(clamp(toInt(e.target.value, 0), 0, 9999))
                          }
                        />
                      </FormField>
                      <FormField label="Max (displays n+ above this)">
                        <TextInput
                          ...
                          value={badgeMax}
                          onChange={(e) =>
                            setBadgeMax(clamp(toInt(e.target.value, 1), 1, 9999))
                          }
                        />
                      </FormField>
                    </>
