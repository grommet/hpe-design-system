...
Building with [semantic design tokens](/design-tokens/overview#semantic-tokens) allows dark mode to be easily applied to an application. The dark mode CSS file is exported as part of the `hpe-design-tokens` package.

To apply dark mode to an application:

1. Import the [dark mode CSS file](#css-variables).
2. Apply `data-mode="dark"` to the root element of the application.
