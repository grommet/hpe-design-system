...
This means HPE digital experiences can be elevated more rapidly as a unified one-HPE. Change can be easy.
...
Semantic tokens can have different values in different contexts. For example, the value of `hpe.color.background.back` might differ for light versus dark mode. Similarly, the value of `hpe.spacing.large` might differ for wide versus narrow viewports. By building with semantic tokens, the values can flex based on the context.
