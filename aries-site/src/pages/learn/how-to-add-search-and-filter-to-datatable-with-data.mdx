...
Use this [CodeSandbox template](https://codesandbox.io/s/hpe-design-system-template-lrdtpe?file=/index.js) to start. The template is populated with a paginated DataTable configured to display SpaceX data.

<Notification
  status="info"
  width="large"
  margin={{ bottom: 'medium' }}
  message="If you’d like to learn more about how to configure a DataTable with a REST API, check out the guide."
  actions={[
    {
      label: 'Guide TBD',
      href: '#',
...
`numberItems` is defined in the starter template using a key from the API response that provides the total number of records in the table.

```
<Data data={data?.docs || []} total={numberItems} toolbar>
    <DataTable columns={columns} />
</Data>
```
...
To ensure the data is updated to reflect changes to the filters, use `view` and `onView`.
