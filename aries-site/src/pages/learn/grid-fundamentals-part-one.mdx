...
boxes. Start by adding `ProductCard` and `productList` to App.js. `ProductCard` will be
used to display the product image, name, and price. `productList` is a JSON file containing
the product data.
...
to `{ count: 'fill', size: '250px' }`.

```js
<Grid columns={{ count: 'fill', size: '250px' }}>...</Grid>
```

Now the Grid will fit as many columns as will fit in the available space, with each column
being a minimum of 250px wide.
...
between the products by adding `gap="medium"` to the Grid.
...
- Use the `fill` value for the `count` property to make the Grid responsive.
- Use the `gap` property to add space between the items.

If you'd like to reference a completed implentation, check out this [CodeSandbox with completed code]().
