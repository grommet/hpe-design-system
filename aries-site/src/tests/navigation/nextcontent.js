...
fixture('NextContent Link').page(`${baseUrl}/foundation/our-brand`);

test(
  'should navigate to correct path when the' + 'next-content is clicked on',
  async t => {
...
test(
  'should navigate to correct path when a next-content' +
    'is choosen via keyboard',
  async t => {
    const page = 'Philosophy';
    const element = Selector('a').withText(page);
    const expectedPath = await element.getAttribute('href');
