...
export class ProductionGuardrailError extends Error {
  constructor(message: string) {
    super(message);
    this.name = 'ProductionGuardrailError';
  }
}
...
export function resolveFigmaSyncConfig(
  ...
  if (isCI && !envArg) {
    throw new Error(
      'Missing required --env value in CI. Allowed values: production, test.',
    );
  }
  ...
  const personalAccessToken = resolveValue({
    ...
    configValue: defaultsForEnv.personalAccessToken,
  ...
  if (!bootstrap) {
    Object.entries(expectedCollectionKeys).forEach(([collection, value]) => {
      if (!value) {
        const missingCollectionKey = [
          `${env.toUpperCase()}_FIGMA_${collection.toUpperCase()}`,
          'COLLECTION_KEY',
        ].join('_');
        missing.push(missingCollectionKey);
      }
    });
  }
  ...
  if (missing.length) {
    ...
    const missingMessagePrefix = `Missing required Figma sync configuration for env "${env}":`;
    throw new Error([missingMessagePrefix, missingKeys].join(' '));
  }
...
export function ensureProductionMutationGuardrails(
  ...
  if (isCI && !allowProdWrites) {
    const blockedMessage = [
      'Production mutation blocked in CI.',
      'Set ALLOW_PRODUCTION_WRITES=true to proceed.',
    ].join(' ');
    throw new ProductionGuardrailError(blockedMessage);
  }

  if (!isCI && !hasConfirmFlag) {
    throw new ProductionGuardrailError(
