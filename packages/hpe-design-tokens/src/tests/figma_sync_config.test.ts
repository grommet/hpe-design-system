...
describe('resolveFigmaSyncConfig', () => {
  ...
  it('resolves from config file defaults when env vars are absent', () => {
    ...
    expect(config.personalAccessToken).toBe('cfg-token');
    expect(config.fileKeys).toEqual({
      primitive: 'cfg-primitive',
      semantic: 'cfg-semantic',
      component: 'cfg-component',
  ...
  it('prefers env vars over config file defaults', () => {
    const tempDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'figma-sync-config-test-'),
    );
    const configFilePath = path.join(tempDir, 'figma-sync.config.json');
    ...
    expect(config.personalAccessToken).toBe('from-env-token');
  });
  ...
  it('requires --env in CI when omitted', () => {
    expect(() =>
      resolveFigmaSyncConfig({
        argv: [],
        env: {
    ...
    ).toThrow('Missing required --env value in CI');
  });

  it('requires --env value in CI when flag has no value', () => {
    expect(() =>
      resolveFigmaSyncConfig({
        argv: ['--env'],
        env: {
