...
async function buildCrossFileAliasLookup(
  ...
  const { aliasLookup: baseLookup, errors: baseLookupErrors } =
    buildAliasLookup(
      {
        ...publishedDep,
  ...
  const { aliasLookup: localOverride, errors: localOverrideErrors } =
    buildAliasLookup(
      {
        ...localVariables,
  ...
  const aliasCollisionErrors = [...baseLookupErrors, ...localOverrideErrors]
    .filter(error => error.code === 'ALIAS_COLLISION')
    .map(error => ({
      code: 'ALIAS_COLLISION' as const,
      message: error.message,
      stage,
      tokenPath: error.tokenPath || 'unknown-alias-target',
      sourceFile: error.sourceFile || `${stage} variables`,
      environment,
      remediation:
        error.remediation ||
        'Ensure alias target names are unique within the environment ' +
          'context for this stage.',
    }));

  if (aliasCollisionErrors.length > 0) {
    aliasCollisionErrors.forEach(error => onAliasResolutionError?.(error));
    throw new Error(
      `Alias lookup collision detected for ${stage}: ` +
        `${aliasCollisionErrors.length} duplicate variable names`,
    );
  }
...
function getCanonicalKeySetForStage(
  localVariables: ApiGetLocalVariablesResponse,
  expectedCollectionKeys: Record<string, string>,
  bootstrap: boolean,
): Set<string> {
  if (!bootstrap) {
    return new Set(Object.values(expectedCollectionKeys));
  }

  const localCollections = localVariables.meta.variableCollections;
  const observedRemoteKeys = Object.values(localVariables.meta.variables)
    .filter(variable => variable.remote)
    .map(variable => localCollections[variable.variableCollectionId]?.key)
    .filter((key): key is string => Boolean(key));

  return new Set(observedRemoteKeys);
}
...
async function main() {
  ...
  try {
    ...
    runErrors.push({
      ...
      remediation: 'Review configuration requirements, then retry.',
    });
  ...
  await FILE_TIERS.reduce<Promise<void>>(async (chainPromise, stage) => {
    ...
    try {
      ...
      const canonicalKeySet = getCanonicalKeySetForStage(
        localVariables,
        config.expectedCollectionKeys,
        config.bootstrap,
      );
