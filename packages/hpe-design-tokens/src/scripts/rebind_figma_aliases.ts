...
function parseTargetCollections(argv: string[]) {
  ...
  if (!parsed.length) {
    throw new Error(
      `Invalid --collections value. Provide a comma-separated list 
      such as color,primitives.`,
    );
  }
  ...
  if (invalid.length) {
    throw new Error(
      `Invalid collection name(s) in --collections: ${invalid.join(', ')}. 
      Allowed values: color, dimension, primitives, global.`,
    );
  }
...
async function applyRebindPlan(
  ...
  for (let i = 0; i < updates.length; i += chunkSize) {
    ...
    await api.postVariables(tierResult.fileKey, { variableModeValues: chunk });
