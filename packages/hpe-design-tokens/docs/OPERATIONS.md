...
It also performs this comparison for root dependency-graph changes to `package.json`,
`pnpm-lock.yaml`, or `pnpm-workspace.yaml`; each revision is installed with its own dependency
graph, and a Changeset is required only when the artifacts differ. The version and changelog
update generated by the release workflow is exempt because it consumes the pending Changeset and
is created by GitHub Actions.
