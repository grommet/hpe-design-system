...
the published `dist` artifacts differ. The version and changelog update generated by the release
workflow is exempt only for the GitHub Actions-created release PR because it consumes the
Changeset that already describes the release.
