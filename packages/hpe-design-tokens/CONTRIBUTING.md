...
`hpe-design-tokens`. For implementation changes or root dependency-graph changes to
`package.json`, `pnpm-lock.yaml`, or `pnpm-workspace.yaml`, the check builds the pull request
merge base and head revisions with their respective dependency graphs and requires a Changeset
only when the published `dist` artifacts differ. The version and changelog update generated by
the release workflow is exempt only for the GitHub Actions-created release PR because it
consumes the Changeset that already describes the release.
