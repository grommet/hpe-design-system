...
### 3. Verify package infrastructure (1 min)
...
│   │
│   ├── orchestrator/
│   │   ├── agent.md                 ← Role, state machine, I/O contract
│   │   └── instructions.md          ← Detailed algorithms, config discovery, state machine
│   │
│   ├── strategist/
│   │   ├── agent.md                 ← Impact/Effort matrix, ranking algorithm, classification
│   │   └── instructions.md          ← Ranking algorithm pseudocode, fallback logic
│   │
│   ├── engineer/
│   │   ├── instructions.md          ← Role, context mgmt, output requirements
│   │   └── skills.md                ← Remediation + generation skills, test matrix, PoC constraints
│   │
│   └── reporter/
│       └── instructions.md          ← Telemetry payload, privacy filters, P1 detection logic
│
...
│   ├── index.ts                     ← Library entry point (exports initializeAgent)
│   └── cli.ts                       ← CLI entry point (audit, fix, generate commands)
│
├── dist/                             ← Compiled JavaScript & type definitions (generated, ignored by .gitignore)
│   ├── index.js & index.d.ts        ← Compiled library
│   ├── cli.js & cli.d.ts            ← Compiled CLI with shebang
│   └── *.d.ts.map                   ← TypeScript declaration maps for IDE support
│
├── package.json                      ← npm config, scripts (build, test, lint, dev)
├── tsconfig.json                     ← TypeScript config (ES2020, strict mode, ESM output)
├── vitest.config.ts                  ← Vitest config (unit & integration tests)
├── .eslintrc.json                    ← ESLint config (TypeScript + Prettier)
└── .gitignore entry: *bin, *dist     ← Build artifacts ignored; checked in src/ instead
```

### Key Notes on Structure

- **Source code lives in `src/`**: All TypeScript goes here
- **Compiled output in `dist/`**: Generated by `pnpm build` (tsc); ignored by .gitignore
- **Binary entry point**: `dist/cli.js` (compiled from `src/cli.ts`); package.json points `"bin": {"hpeds-ai": "./dist/cli.js"}`
- **No `bin/` directory**: Source code doesn't belong in build artifact folders; pnpm automatically registers CLI when package is installed
...
# Run tests (when test suite is added in PoC Workstream 1)
pnpm test
...
When ready to publish to npm (MVP+):

```bash
# From root, using changesets
cd /Users/glissman/Code/hpe-design-system
pnpm changeset
...
- Package is auto-registered via `packages/**` glob in `pnpm-workspace.yaml`
- Shared devDependencies (typescript, eslint, prettier) managed via catalog in `pnpm-workspace.yaml`
- Root `pnpm build` automatically builds design-system-agent (and all other packages)
- Monorepo-level scripts:
  ```bash
  pnpm build          # builds all packages, including design-system-agent
  pnpm build:agent    # (optional; not yet added to root package.json)
  ```
...
### 9. Workstream 1 (Orchestrator) Unblocked: Package Infrastructure Complete

**Status:** ✅ Package bootstrap finished (March 1, 2026)

- `package.json` with TypeScript, build, test, lint scripts
- `tsconfig.json` configured for Node.js/CLI (ES2020, strict mode, ESM)
- `vitest.config.ts` ready for unit/integration tests
- `.eslintrc.json` extends monorepo config
- CLI executable: `node dist/cli.js --help` works

**Next:** Begin Workstream 1 implementation (Orchestrator state machine, config discovery, approval gating) per [PoC-IMPLEMENTATION.md](PoC-IMPLEMENTATION.md) Workstream 1 plan.
...
Before creating a pull request or committing infrastructure changes:

- [ ] **Build succeeds:** `pnpm build` produces no errors
- [ ] **CLI works:** `node dist/cli.js --help` shows usage without errors
- [ ] **Linting passes:** `pnpm lint` returns exit code 0
- [ ] **Types correct:** `pnpm type-check` finds no type errors
- [ ] **No warnings:** Ignore deprecation warnings from dependencies (expected in monorepo)
- [ ] **Git clean:** All build artifacts in `dist/` are .gitignored (verified by `git status`)
- [ ] **Commit message:** Use conventional commits (e.g., `feat: add Orchestrator state machine` or `fix: context budget overflow in Auditor`)
