 ...
 * Works on raw text, so it covers .js/.jsx/.ts/.tsx/.mjs/.cjs/.mts/.cts
...
// Authored source roots across the monorepo (relative to repo root).
const ROOTS = ['apps', 'packages', 'shared', 'sandbox', 'scripts'];
...
function main() {
  ...
  if (stagedOnly) {
    ...
    const roots =
      targets.length > 0
        ...
        : ROOTS.map(root => path.join(ROOT, root));
    files = [];
  ...
  if (stagedOnly && !checkOnly && nonCompliant.length > 0) {
    execFileSync('git', ['add', '--', ...nonCompliant], { cwd: ROOT });
  }
  ...
  if (checkOnly) {
    if (nonCompliant.length > 0) {
      ...
      console.error('\nRun "pnpm license" to fix the headers.');
      process.exit(1);
    }
