...
<Example
  ...
  code="https://raw.githubusercontent.com/grommet/hpe-design-system/master/apps/aries-site/src/examples/components/selectmultiple/SelectMultipleSimpleExample.js"
...
To properly associate FormField `label` with Grommet's SelectMultiple component, ensure the SelectMultiple has an `id` and the FormField has an `htmlFor` with the format of `${id}__input`.

```
<FormField label="Select option" htmlFor="selectmultiple__input">
  <SelectMultiple id="selectmultiple" options={['Option 1', 'Option 2']} />
</FormField>
```
