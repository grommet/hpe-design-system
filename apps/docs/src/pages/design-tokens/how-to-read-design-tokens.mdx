...
| **target**    | Where a color is applied.                                                                                                                                                                                          | background, border, text, icon, foreground     | Semantic color tokens only.                                                                                                                                  |
| **role**      | The semantic meaning of a color.                                                                                                                                                                                    | primary, critical, selected.primary           | Some roles have two parts: a family followed by a name. |
...
Semantic color tokens name the **target** (the part of an element being styled)
and the **role** (its semantic meaning), followed by optional **scale** and **state**.

Most roles are a single name, such as `primary` or `critical`. Some roles have
two parts: a **family** followed by a name, such as `accent.blue` or
`selected.primary`.

<Example height="auto">
  <TokenHighlight
    highlight={['prefix', 'type', 'target', 'role', 'scale', 'state']}
  />
</Example>

| prefix | type      | target     | role             | scale  | state | Token                                     |
| ------ | --------- | ---------- | ---------------- | ------ | ----- | ----------------------------------------- |
| hpe    | color     | background | primary          | strong | hover | hpe.color.background.primary.strong.hover |
| hpe    | color     | background | accent.blue      | --     | --    | hpe.color.background.accent.blue          |
| hpe    | color     | background | selected.primary | --     | --    | hpe.color.background.selected.primary     |
| hpe    | color     | background | accent.blue      | weak   | --    | hpe.color.background.accent.blue.weak     |
| hpe    | color     | text       | placeholder      | --     | --    | hpe.color.text.placeholder                |
...
Only `type` is required. `target`, `role`, `scale` and `state` are each
optional, as the non-color rows above show.

### Component

Component tokens represent tokens for specific components and should not be used to style a different component.

Component tokens use `variant` where semantic tokens use `role`. These are
separate schemas and the terms are not interchangeable.
