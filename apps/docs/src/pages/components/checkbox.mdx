...
### CheckBox with FormField

When wrapping a CheckBox in a FormField, do not provide a `label` prop to FormField. Because CheckBox already has its own `label` prop that visually and programmatically labels the input, adding a FormField `label` creates multiple label elements for the same input, resulting in accessibility errors.

Use FormField with CheckBox when you need to display help text, error messages, or validation for the checkbox.
...
CheckBox's own `label` prop provides the accessible label for the input. The label on each checkbox should clearly describe its purpose.

When using CheckBox inside a FormField, do not also set a `label` on the FormField. Doing so creates two label elements for the same input, resulting in multiple label accessibility errors. Use FormField without a `label` when you need help text, error messages, or validation alongside a CheckBox.
