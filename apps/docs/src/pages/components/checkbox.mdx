...
When a CheckBox is used without a `label` prop, ensure the input is still accessible by providing an `aria-label` or `aria-labelledby` attribute that describes its purpose.
