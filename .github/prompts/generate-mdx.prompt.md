...
5. **Anatomy Section:** When generating the `## Anatomy` section, follow `.github/instructions/anatomy-diagram.instruction.md` for all rules. The section always contains two parts in order:
   - An `<Example caption="Diagram illustrating the parts composing [Component]." plain><[Component]Anatomy /></Example>` block. The anatomy component must be imported at the top of the MDX file with the other example imports: `import { [Component]Anatomy } from '../../examples';`.
   - An annotation table immediately after, with no additional headings or prose between the `<Example>` block and the table. Columns: `Label`, `Region`, `Purpose`, `Required`, `Notes`. For `availability: 'required'` entries, use `<span><Checkmark a11yTitle="true" size="small" /></span>`. For `availability: 'optional'` entries, write `Optional`. Ensure `Checkmark` is imported: `import { Checkmark } from '@hpe-design/icons-grommet';`.
...
7. Ensure that `contentGuidelines` and `dosAndDonts` objects from the YAML configuration are correctly translated into the `## Content Guidelines` and `## Dos and Do Nots` sections using appropriate lists or `<BestPracticeGroup>` layouts as appropriate (do not mock real code, just content). **For `## Dos and Don'ts`:** render each `dosAndDonts` entry as a `<BestPracticeGroup>` with a `do` and `dont` `<Example bestPractice>` pair. Do NOT add a `###` subheading or prose description above each pair — the `message` prop is the only label. `<BestPracticeGroup>` blocks sit directly under the section heading. When no real example component exists yet, use `<div>{/* TODO: ... */}</div>` as the child — never a bare JSX comment, as that produces `undefined` children and crashes `cloneElement`.
